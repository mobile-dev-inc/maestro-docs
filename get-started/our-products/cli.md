...
The most critical use case for the command-line tool is automation. While Maestro Studio is ideal for _creating_ tests, the command-line tool is the standard tool for _running_ those tests recurrently and autonomously.
...
* **Utility Commands:** The command-line tool includes helper tools, such as the `download-samples` command, which downloads project examples (like Wikipedia) for quick learning, and specific commands for managing _workspace_ configurations.

## Command-line tool vs. Studio

It's important to understand that Studio acts as a _wrapper_ (a layer) over the command-line tool. Studio depends on the command-line tool to function.
