...
3. Add `ANDROID_HOME` to Windows `PATH`. a. To check if your `ANDROID_HOME` is on the Windows `PATH`, open a PowerShell terminal and run this command `adb --version.`
4. Install Java JDK 17 or higher and set `JAVA_HOME`. a. Run `java -version` to check if Java 17+ is installed correctly.
