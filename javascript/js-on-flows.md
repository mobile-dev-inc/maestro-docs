...
{% hint style="info" %}
You can directly access `env` parameters from within JavaScript. See [parameters-and-constants](https://docs.maestro.dev/advanced/parameters-and-constants "mention") for more information.
{% endhint %}
...
The `runScript` accepts `env` parameters, in the same way as `runFlow` does (see [nested-flows](https://docs.maestro.dev/advanced/nested-flows "mention")).
...
|:-------:|:------|
| `copiedText` | Contains the result of the [`copyTextFrom`](https://docs.maestro.dev/api-reference/commands/copytextfrom) command. <br> Access [Access element text](https://docs.maestro.dev/advanced/javascript/access-element-text) for details. |
